  ...
  >
      ...
      <Container 
        ...
        <Grid container spacing={2} display="flex" alignContent="center" alignItems="center" justifyContent="center" marginBottom={2}>
          ...
          <Grid item>
            <Button appearance="primary" color="primary" onClick={runOptimizer} disabled={isOptimizerRunning}>
              Run Optimizer
            </Button>
            {isOptimizerRunning && <CircularProgress size={24} sx={{ ml: 1 }} />}
          </Grid>
          <Grid item>
            <Button appearance="primary" color="secondary" onClick={stopOptimizer} disabled={!isOptimizerRunning}>
              Stop Optimizer
            </Button>
          </Grid>
        </Grid>
...
export default function ToggleColorModeApp() {
  ...
  return (
    ...
    <FluentProvider theme={isDarkMode ? customDarkTheme : customLightTheme}>
      <button onClick={() => setMode(prevMode => prevMode === 'light' ? 'dark' : 'light')}>
        Toggle Theme
      </button>
