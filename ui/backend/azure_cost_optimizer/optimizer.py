...
def process_subscription(subscription, mode, summary_reports, impacted_resources, non_impacted_resources, status_log, start_date, end_date):
    ...
    try:
        ...
        if mode == 'apply':
            ...
            logger.info(f'Dry run mode, policies will not be applied to subscription: {subscription_id}')
            apply_policies(policies, dry_run=True, subscription_id=subscription_id, impacted_resources=impacted_resources, non_impacted_resources=non_impacted_resources, status_log=status_log)
    except Exception as e:
