...
def calculate_unattached_disks_cost():
    """Calculate the cost of unattached disks for the current month."""
    unattached_disks_cost = 0.0
    unattached_disks = []

    disks = compute_client.disks.list()
    for disk in disks:
        if unattached_filter(disk):
            unattached_disks.append(disk)
            resource_group_scope = f'/subscriptions/{subscription_id}/resourceGroups/{disk.id.split("/")[4]}'
            cost_data = get_cost_data(resource_group_scope)
            if cost_data:
                filtered_data = filter_cost_data(cost_data, disk.id)
                for item in filtered_data:
                    unattached_disks_cost += item[0]
    
    return unattached_disks_cost, unattached_disks

def calculate_unattached_public_ips_cost():
    """Calculate the cost of unattached public IPs for the current month."""
    unattached_pips_cost = 0.0
    unattached_pips = []

    public_ips = network_client.public_ip_addresses.list_all()
    for pip in public_ips:
        if unattached_filter(pip):
            unattached_pips.append(pip)
            resource_group_scope = f'/subscriptions/{subscription_id}/resourceGroups/{pip.id.split("/")[4]}'
            cost_data = get_cost_data(resource_group_scope)
            if cost_data:
                filtered_data = filter_cost_data(cost_data, pip.id)
                for item in filtered_data:
                    unattached_pips_cost += item[0]
    
    return unattached_pips_cost, unattached_pips
